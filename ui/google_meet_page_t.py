...
class GoogleMeetPage(QWidget):
    def __init__(self, dashboard, access_token):
        ...
        self.trainer_name_label = QLabel("Fetching Trainer Name...")
        self.trainer_name_label.setAlignment(Qt.AlignCenter)
        self.layout.addWidget(self.trainer_name_label)

        self.fetch_trainer_info()
    ...
    def fetch_trainer_info(self):
        try:
            ...
            trainer_name = profile_data.get("username", "Trainer Name")
            self.trainer_name_label.setText(f"Trainer: {trainer_name}")
        except requests.exceptions.RequestException as e:
            QMessageBox.critical(self, "Error", f"Could not fetch trainer info: {e}")

    def fetch_premium_users(self):
        try:
            api_url = "http://127.0.0.1:5000/meeting/get_premium_users"
            headers = {"Authorization": f"Bearer {self.access_token}"}
            response = requests.get(api_url, headers=headers)
            response.raise_for_status()
            users = response.json()
    ...
    def schedule_meeting(self):
        ...
        try:
            api_url_profile = "http://127.0.0.1:5000/trainerprofile/profile"
            headers = {"Authorization": f"Bearer {self.access_token}"}
            profile_response = requests.get(api_url_profile, headers=headers)
            profile_response.raise_for_status()
            profile_data = profile_response.json()

            trainer_id = profile_data.get("_id", None)
            trainer_name = profile_data.get("username", "Trainer Name")

            if not trainer_id:
                QMessageBox.critical(self, "Error", "Could not retrieve trainer ID.")
                return

            api_url = "http://127.0.0.1:5000/meeting/schedule_meeting"
            data = {
                "user_id": user_id,
    ...
    def fetch_previous_meetings(self):
        try:
            ...
            trainer_id = profile_data.get("_id", None)
            if not trainer_id:
                QMessageBox.critical(self, "Error", "Could not trainer ID.")
                return

            api_url_meetings = f"http://127.0.0.1:5000/meeting/get_previous_meetings/{trainer_id}"
