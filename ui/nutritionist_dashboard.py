...
import json
from ui.google_meet_page_n import GoogleMeetPageN

class NutritionistProfileView(QWidget):
...
class NutritionistDashboard(QWidget):
    def __init__(self, nutritionist_data=None, main_window=None, parent=None):
        super().__init__(parent)
        self.icon_dir = os.path.dirname(os.path.abspath(__file__))
        self.main_window = main_window
        self.nutritionist_data = nutritionist_data
        self.setWindowTitle("Nutritionist Dashboard")
        self.setGeometry(100, 100, 800, 600)

        # Initialize views first
        self.profile_view = NutritionistProfileView()
        self.edit_profile_widget = self.create_edit_profile_form()
        self.view_profile_widget = NutritionistProfileView()
        ...
        self.google_meet_page_n = GoogleMeetPageN(self, self.main_window.access_token)
        self.content_area_widget.addWidget(self.google_meet_page_n)
        ...
        print("NutritionistDashboard __init__ completed")
    ...
    def create_sidebar(self):
        ...
        self.nutritionist_name_label = QLabel()
        self.nutritionist_name_label.setAlignment(Qt.AlignCenter)
        self.nutritionist_name_label.setStyleSheet("font-size: 18px; font-weight: bold; color: white;background-color: transparent;")
        header_layout.addWidget(self.nutritionist_name_label)
        ...
        self.google_meet_button.clicked.connect(self.show_google_meet_page_n)
        sidebar_layout.addWidget(self.google_meet_button)

        sidebar_layout.addStretch()
        self.set_nutritionist_info()
    ...
    def go_back_to_login(self):
        if self.main_window:
            ...
            self.main_window.open_page(Login2Page, role="Nutritionist")

        
    def show_google_meet_page_n(self):
     self.google_meet_page_n.populate_meet_details()
     self.content_area_widget.setCurrentWidget(self.google_meet_page_n)
    ...
    def set_nutritionist_info(self):
        if self.nutritionist_data:
            name = self.nutritionist_data.get("username", "Nutritionist Name")
            self.nutritionist_name_label.setText(name)
            ...
            self.nutritionist_name_label.setText("Nutritionist Name")
            self.profile_picture_label.setText("No Picture")
    ...
    def populate_edit_form(self):
        if self.nutritionist_data:
            self.name_input.setText(self.nutritionist_data.get("username", ""))
            self.email_input.setText(self.nutritionist_data.get("email", ""))
            self.phone_input.setText(self.nutritionist_data.get("phone", ""))
            self.age_input.setText(str(self.nutritionist_data.get("age", "")))
            self.experience_input.setText(str(self.nutritionist_data.get("experience", "")))
            self.gender_input.setText(self.nutritionist_data.get("gender", ""))
            self.specialization_input.setText(
                self.nutritionist_data.get("specialization", "")
            )
            self.degree_input.setText(self.nutritionist_data.get("degree", ""))
            self.location_input.setText(self.nutritionist_data.get("location", ""))
            self.role_input.setText(self.nutritionist_data.get("role", ""))
    ...
    def show_view_profile(self):
        ...
        self.view_profile_widget.set_profile_data(self.nutritionist_data)
        self.content_area_widget.setCurrentWidget(self.view_profile_widget)
        print("show_view_profile finished")
    ...
    def save_profile(self):
        ...
        try:
            api_url = "http://127.0.0.1:5000/nutriprofile/editprofile"
            headers = {
    ...
    def fetch_and_display_profile(self):
        ...
        try:
            ...
            self.nutritionist_data = profile_data
